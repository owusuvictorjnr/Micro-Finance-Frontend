...
const SidebarFooter: React.FC<SidebarFooterProps> = ({ isCollapsed = false }) => {
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === "ArrowDown" || e.key === "ArrowUp") {
      e.preventDefault();
      const focusable = menuRef.current?.querySelectorAll<HTMLButtonElement | HTMLAnchorElement>(
        'button:not([disabled]), a:not([disabled])'
      );
      if (!focusable || focusable.length === 0) return;
      const list = Array.from(focusable);
      const index = list.indexOf(document.activeElement as HTMLButtonElement | HTMLAnchorElement);
      
      let nextIndex = index;
      if (e.key === "ArrowDown") {
        nextIndex = index === -1 ? 0 : (index + 1) % list.length;
      } else if (e.key === "ArrowUp") {
        nextIndex = index === -1 ? list.length - 1 : (index - 1 + list.length) % list.length;
      }
  ...
  return (
    <div
      ref={containerRef}
      className={`relative border-t border-zinc-200 p-4 dark:border-zinc-800/60 bg-white dark:bg-[#0B0F19] flex flex-col gap-1 transition-all duration-300 ${isCollapsed ? "items-center" : ""}`}
    >
      ...
      <button
        ...
        aria-haspopup="menu"
        aria-controls={isExpanded ? "user-profile-menu" : undefined}
