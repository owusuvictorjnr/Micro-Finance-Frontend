...
import { useRecentApplicationsQuery, useApproveApplicationMutation } from "../../hooks/use-overview-queries";
import {
  PendingIcon,
  ApprovedIcon,
  RejectedIcon,
...
export const RecentApplications: React.FC<RecentApplicationsProps> = ({
  ...
  const getAiRating = (score: number) => {
    if (score >= 800) return { letter: "A+", color: "bg-emerald-500/15 text-emerald-500 dark:bg-emerald-500/10 dark:text-emerald-400 border border-emerald-500/20" };
    if (score >= 700) return { letter: "A", color: "bg-emerald-500/15 text-emerald-500 dark:bg-emerald-500/10 dark:text-emerald-400 border border-emerald-500/20" };
    if (score >= 600) return { letter: "B", color: "bg-amber-500/15 text-amber-500 dark:bg-amber-500/10 dark:text-amber-400 border border-amber-500/20" };
    return { letter: "C", color: "bg-rose-500/15 text-rose-500 dark:bg-rose-500/10 dark:text-rose-400 border border-rose-500/20" };
  };
          ...
          <tbody className="text-sm font-medium text-zinc-700 dark:text-zinc-300">
            {isLoading ? (
              ...
              filteredApplications.map((app, index) => {
                const isMutating = approveMutation.isPending && approveMutation.variables === app.id;
                ...
                return (
                  <tr key={`${app.id}-${index}`} className="group hover:bg-zinc-50/50 dark:hover:bg-zinc-800/10 transition-colors">
                    ...
                    <td className="py-4 px-2">
                      <div className="font-semibold text-zinc-900 dark:text-zinc-100 text-sm">{app.amount}</div>
                      <div className="text-xs text-zinc-400 dark:text-zinc-500 font-medium mt-0.5">{app.term}</div>
                    </td>

                    {/* Type badge */}
                    <td className="py-4 px-2">
                      <span className={`inline-block rounded px-2.5 py-1 text-xs font-bold tracking-wide ${getTypeBadgeStyles(app.type)}`}>
                    ...
                    <td className="py-4 px-2">
                      <div className="flex items-center gap-2.5">
                        <span className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-xs font-bold ${aiRating.color}`}>
                          {aiRating.letter}
                        </span>
                        <span className="font-semibold text-zinc-900 dark:text-zinc-100 text-sm">{app.aiScore}</span>
                      </div>
                    </td>
